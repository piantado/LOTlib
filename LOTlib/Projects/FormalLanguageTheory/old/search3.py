...
class AugustHypothesis(RegenerationProposal, StochasticLikelihood, RecursiveLOTHypothesis):
    def __init__(self, grammar=base_grammar, **kwargs):
        RecursiveLOTHypothesis.__init__(self, grammar, max_recurse=5, **kwargs)
    ...
    def p_propose(self, n):
        # We'll only permit proposals to nonabstract functions, in order to try to search ovr abstractions
        return 1*(n.returntype in {'LIST', 'ATOM', 'BOOL'})

    def propose(self):
        return RegenerationProposal.propose(self, resampleProbability=self.p_propose)
    ...
    def compute_single_likelihood(self, datum):
        ...
        for k in datum.output.keys():
            ll += datum.output[k] * (log(llcounts.get(k, 1.0e-9)) - log(lo))
