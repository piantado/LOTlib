...
keepers <- unique(subset(d, posterior.score > -55)$h) # keep any hypothesis that gets above 1% posterior prob
...
# color by hypothesis
plt <- ggplot(d2, aes(x=data.size, y=posterior.score, color=h)) +  geom_line()
...
d <- read.table("G://programs and files//Python//Lib//site-packages//LOTlib//LOTlib//Examples//FormalLanguageTheory//Simulation//stage")
