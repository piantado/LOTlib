...
def sample_context_set():
	...
	# for the number-style probabilities
...
def generate_data(data_size):
	...
	for i in xrange(data_size):
		...
		#print map(lambda x: x.word, all_utterances)
		
		u = target.sample_utterance(all_utterances)
		
		#print "SAMPLING u=",u.word, '\n\n'
...
def my_gricean_weight(h, f):
	...
	"""
	#return 1.0
	pct = float(sum(map(lambda s: ifelse(f(*s), 1.0, 0.0), TESTING_SET) )) / len(TESTING_SET)
...
def show_baseline_distribution(N=10000):
	...
	for w in frq.keys():
		...
		pct = float(sum(map(lambda s: ifelse(target.dfunc[w](*s), 1.0, 0.0), TESTING_SET) )) / len(TESTING_SET) 
...
nu = 10.0 ## Note: The max weight is 1/nu, and this should not be huge compared to 1/alpha
...
#target.force_function('a',     lambda A, B, S: presup_(True, cardinality1_(intersection_(A,B))))
