...
parser.add_option("--in", dest="IN_PATH", type="string",
                  ...
                  default="/home/piantado/Desktop/mit/Libraries/LOTlib/examples/Quantifier/data/all_trees_2012Feb16.pkl")
...
class GibbsyGriceanVectorizedLexicon(Hypothesis):
	...
	def compute_likelihood(self, *args):
		...
		if r.shape[1] == 0:  # if no data
			...
			# this gives a vector of log likelihoods to each possible set
			lp = np.log( options.PALPHA*options.ALPHA*weights[self.word_idx[uttered_word_index]]*(rutt>0) / (1e-5 + true_weights) + \
			...
			# which is dotted with the count of how often each set occurs
			print lp.shape
			print data_counts.shape
			lp = sum(lp*data_counts)
...
def run(data_size):
	...
	# how often each "possible" set occurs
	data_counts = np.random.multinomial(data_size, [1.0/float(len(all_possible_context_sets))] * len(all_possible_context_sets) )
	...
	#print all_possible_utterances
	response = np.array( [ mapto012(get_single_tree_responses(t, all_possible_utterances, utterance=True)) for t in my_finite_trees] )
	...
	uttered_word_index = [word2index[u.word] for u in all_possible_utterances]
	
	# vector 0,1,2, ... number of columsn
	#zerothroughcols = np.array(range(data_size))
	zerothroughcols = np.array(range(len(all_possible_utterances)))
	...
	for g in LOTlib.MetropolisHastings.gibbs_sample(learner, [], options.STEPS, dimensions=xrange(len(target.all_words()))):
...
all_possible_utterances = make_all_SimpleUtterances(target.all_words(), all_possible_context_sets)
