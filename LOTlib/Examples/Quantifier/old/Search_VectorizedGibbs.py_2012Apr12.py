...
if isinstance(fs.peek()[0], FunctionNode): # if we store trees in each
	...
	#print "HERE1"
elif isinstance(fs.peek()[0], GibbsyGriceanVectorizedLexicon) or isinstance(fs.peek()[0], SimpleLexicon): # else we want to extract the treesed from this priority queue
	upq = UniquePriorityQueue(N=-1)
	for l in fs.get_all():
		for e in l.dexpr.values():
			upq.push(e, 0.0)
	my_finite_trees = upq.get_all()
	#print "HERE2"
