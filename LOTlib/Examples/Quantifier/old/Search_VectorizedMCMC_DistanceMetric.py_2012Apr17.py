...
parser.add_option("--in", dest="IN_PATH", type="string", help="Input file (a pickle of UniquePriorityQueue)", 
                  default="/home/piantado/Desktop/mit/Libraries/LOTlib/examples/Quantifier/data/all_trees_2012Apr17.pkl")
...
def run(data_size):
	...
	for g in LOTlib.MetropolisHastings.mh_sample(learner, databundle, options.STEPS, skip=options.SKIP):
...
#proposal_probs = {}
#response_cache = {}
#proposal_Z = {} # the normalizing constant for each row
#data = generate_data(100)
#for x in my_finite_trees: 
	#response_cache[x] = numpy.array(mapto012(get_single_tree_responses(x, data, data=True)))

## compute distance between hypotheses, giving partial credit to T/F and Undef/def
#def mydistance(x,y,scale=0.10):
	#rca = response_cache[a]
	#rcb = response_cache[b]
	#d = 2.0*numpy.mean(rca != rcb) + numpy.mean(collapse_undef(rca) != collapse_undef(rcb)) + numpy.mean(is_undef(rca) != is_undef(rcb))
	#return exp(-scale * d)

#for a in my_finite_trees:
	#lst = []
	#for b in my_finite_trees:
		#if b is a: continue # don't propose to yourself, idiot
		#d = mydistance(a,b)
		#proposal_Z[a] = proposal_Z.get(a,0.0) + d
		#lst.append( ( d, b) )
	#lst.sort(key=lambda x:x[0], reverse=True)
	#proposal_to[a]   = map(lambda x:x[1], lst)
	#proposal_probs[a] = map(lambda x:x[0], lst)
...
def distance_based_proposer(x):
	y = randint(0, len(my_finite_trees)-1 )
	return y, 0.0

dprintn(5, "# Done computing distances")
