...
G.add_rule('BOOL', 'exhaustive_', ['SET', 'context.S'], 1.0)
...
G.add_rule('SET', 'context.A', None, 10.0)
G.add_rule('SET', 'context.B', None, 10.0)
G.add_rule('SET', 'context.S', None, 10.0) ## Must include this or else we can't get complement
...
for w, f in target_functions.items():
	target.set_word(w, LOTHypothesis(G, value='SET_IN_TARGET', f=f))
